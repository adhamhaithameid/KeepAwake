...
struct CleanTabView: View {
                ...
                ZStack {
                    Circle()
                        .fill(model.isKeyboardLocked
                              ? KeepAwakePalette.blue.opacity(0.15)
                              : KeepAwakePalette.surface)
                        .frame(width: 48, height: 48)
                        .overlay {
                            Circle()
                                .strokeBorder(model.isKeyboardLocked
                                              ? KeepAwakePalette.blue.opacity(0.4)
                                              : KeepAwakePalette.border, lineWidth: 1)
                        }

                    Image(systemName: model.isKeyboardLocked ? "keyboard.fill" : "keyboard")
                        .font(.system(size: 20, weight: .medium))
                        .foregroundStyle(model.isKeyboardLocked ? KeepAwakePalette.blue : KeepAwakePalette.ink)
                }
                ...
                Text(model.isKeyboardLocked ? "Re-enable" : "Disable")
                    ...
                    .background {
                        RoundedRectangle(cornerRadius: 8, style: .continuous)
                            .fill(model.isKeyboardLocked
                                  ? KeepAwakePalette.success
                                  : KeepAwakePalette.blue)
                    }
            }
            .buttonStyle(.plain)
                ...
                ZStack {
                    Circle()
                        .fill(model.isTimedSessionActive
                              ? KeepAwakePalette.orange.opacity(0.15)
                              : KeepAwakePalette.surface)
                        .frame(width: 48, height: 48)
                        .overlay {
                            Circle()
                                .strokeBorder(model.isTimedSessionActive
                                              ? KeepAwakePalette.orange.opacity(0.4)
                                              : KeepAwakePalette.border, lineWidth: 1)
                        }

                    Image(systemName: model.isTimedSessionActive ? "lock.fill" : "lock.open")
                        .font(.system(size: 20, weight: .medium))
                        .foregroundStyle(model.isTimedSessionActive ? KeepAwakePalette.orange : KeepAwakePalette.ink)
                }
    ...
    private func timerDisplay(remaining: Int) -> some View {
        ...
        return HStack(spacing: 14) {
            ZStack {
                Circle()
                    .stroke(KeepAwakePalette.border.opacity(0.3), lineWidth: 5)
        ...
        .background {
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(KeepAwakePalette.orange.opacity(0.06))
                .overlay {
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(KeepAwakePalette.orange.opacity(0.2), lineWidth: 1)
                }
        }
        .transition(.opacity)
    }
    ...
    private func autoStartBanner(countdown: Int) -> some View {
        ...
        .background {
            RoundedRectangle(cornerRadius: 8, style: .continuous)
                .fill(KeepAwakePalette.blue.opacity(0.08))
                .overlay {
                    RoundedRectangle(cornerRadius: 8, style: .continuous)
                        .strokeBorder(KeepAwakePalette.blue.opacity(0.2), lineWidth: 1)
                }
        }
    }
    ...
    private var statusBar: some View {
        HStack(spacing: 6) {
            Circle()
                .fill(model.activeSession != nil ? KeepAwakePalette.orange : KeepAwakePalette.success)
