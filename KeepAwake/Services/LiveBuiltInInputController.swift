...
private let logger = Logger(subsystem: "com.adhamhaithameid.keepawake", category: "InputBlocking")
...
actor LiveBuiltInInputController: BuiltInInputControlling {
    ...
    func availabilitySummary() async -> String {
        ...
        case (true, true, false):
            return "Allow KeepAwake in Privacy & Security \u{2192} Accessibility, then reopen it to start cleaning."
        case (true, false, _):
    ...
    func lock(target: BuiltInInputTarget) async throws -> InputLockLease {
        ...
        do {
            ...
            guard KeyboardBlockerResource.isTrusted(prompt: true) else {
                ...
                throw KeepAwakeError.permissionDenied(
                    "Allow KeepAwake in Privacy & Security \u{2192} Accessibility. Enable it, then reopen the app and try again."
                )
            }
            ...
            if !hasInputMonitoring {
                ...
                throw KeepAwakeError.permissionDenied(
                    "Allow KeepAwake in Privacy & Security \u{2192} Input Monitoring. Enable it, then try again."
                )
            }
...
private final class TrackpadBlockerResource: NSObject, InputLockResource {
    ...
    private func start() {
        ...
        thread.name = "KeepAwake.TrackpadBlocker"
        thread.start()
    }
    ...
    private func runTapLoop() {
        ...
        guard let tap = CGEvent.tapCreate(
            ...
            setupError = KeepAwakeError.permissionDenied(
                "Allow KeepAwake in Accessibility to block the trackpad."
            )
...
private final class KeyboardBlockerResource: NSObject, InputLockResource {
    ...
    private func start() {
        ...
        thread.name = "KeepAwake.KeyboardBlocker"
        thread.start()
    }
    ...
    private func runTapLoop() {
        ...
        guard let tap = createdTap else {
            ...
            setupError = KeepAwakeError.permissionDenied(
                "Allow KeepAwake in Accessibility and Input Monitoring to block the keyboard."
            )
            readySemaphore.signal()
            return
        }
